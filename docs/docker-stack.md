...
OSS + proprietary backend overlay:
...
```bash
docker compose -f docker-compose.oss.yml -f docker-compose.proprietary.yml up -d admin-service jobs-service billing-service ingest-service
```

Start frontend surfaces locally (recommended for Cloudflare-style deployment):

```bash
./scripts/frontend-services.sh start
```

Frontend directories (deployed separately in production):

- `frontend/web/` (main site)
- `frontend/admin/` (admin site)
- `frontend/cloud/` (tenant app site)
- `frontend/demo/` (demo site)
...
- Admin Website: `http://localhost:3006/login/`
- Cloud Website: `http://localhost:3007/login/`
...
- Admin CORS is restricted by `ADMIN_CORS_ORIGIN` (default `http://localhost:3005,http://localhost:3006,http://localhost:3007,http://localhost:3008`).
