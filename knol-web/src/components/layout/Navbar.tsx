...
const KNOL_LOGO = (
  <svg width="32" height="32" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Knol home" role="img">
    ...
    <header className="fixed top-0 w-full bg-dark-900/95 backdrop-blur-md border-b border-dark-600/30 z-50">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        ...
        <ul className="hidden md:flex items-center gap-8">
          ...
          <li>
            <a href={SITE.appUrl} className="btn-primary text-sm !py-2 !px-4">Get Started Free</a>
          </li>
        </ul>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-dark-800 border-t border-dark-600/30 px-4 py-4 space-y-3">
          ...
          <a href={SITE.appUrl} className="block btn-primary text-sm text-center mt-2" onClick={() => setMobileOpen(false)}>Get Started Free</a>
        </div>
      )}
    </header>
  );
