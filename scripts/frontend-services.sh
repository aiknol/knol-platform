...
Commands:
  start    Start all frontend services (main, admin, app, demo).
...
has_next_binary() {
  [ -x "$MAIN_WEB_DIR/node_modules/.bin/next" ] || [ -x "$FRONTEND_DIR/node_modules/.bin/next" ]
}
...
stop_frontends() {
  for pid_file in /tmp/web-main-3005.pid /tmp/admin-3006.pid /tmp/app-3007.pid /tmp/demo-3008.pid; do
    if [ -f "$pid_file" ]; then
...
show_status() {
  local ports=(3005 3006 3007 3008)
  local labels=("Main" "Admin" "App" "Demo")
  local i
...
clean_next_caches() {
  local dirs=("$MAIN_WEB_DIR" "$ADMIN_WEB_DIR" "$APP_WEB_DIR" "$DEMO_DIR")
  local dir
  for dir in "${dirs[@]}"; do
    if [ -d "$dir/.next" ]; then
      rm -r "$dir/.next"
